...
class Edge_Operators(nn.Module):
    def __init__(self, input_channel, the_filter='Isotropic_Sobel', mode='filtering', parameter_a=1, parameter_k=0.5,
        ...
        Args:
            ...
            the_filter (str): Decide which filter to use ('Isotropic_Sobel','Krisch','Laplacian','LOG'.).
        ...
        """
        super(Edge_Operators, self).__init__()
        ...
        if self.filter == 'Isotropic_Sobel':
            ...
            kernel_1 = torch.FloatTensor(kernel_laplacian).expand(self.channel, self.channel, 3, 3)
            self.weight_1 = nn.Parameter(data=kernel_1, requires_grad=False)
